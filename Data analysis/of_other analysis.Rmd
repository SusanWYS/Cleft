...
data1_freqs = c(0.871421,0.142633,0.401192,0.690923,0.122813,0.366481,0.707127,0.449262,0.996130,0.107783,0.990297,0.191637,
               0.057329,0.7795,0.020775,0.148373,0.011884,0.078544,0.09882,0.161307,0.195696,0.039979,0.166375,0.086595)

for (x in 1:12) {
  filtered_study <- filtered_study %>% 
  mutate(freq = ifelse(item == x & experiment == "V-Prep-NP", data1_freqs[x], freq))
  filtered_study <- filtered_study %>% 
  mutate(freq = ifelse(item == x & experiment == "V-P-NP", data1_freqs[x+12], freq))
}
```
